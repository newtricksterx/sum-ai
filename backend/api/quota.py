...
class QuotaExceeded(Exception):
    ...
    def __init__(self, subscription: Subscription):
        super().__init__("summary_limit_reached")
...
def reserve_request_slot(user) -> tuple[Subscription, int | None]:
    ...
    is at or above their plan's summary_limit. Resets the usage period first
    ...
    with transaction.atomic():
        ...
        limit = subscription.summary_limit
        if limit is not None and subscription.summaries_used >= limit:
            raise QuotaExceeded(subscription)

        Subscription.objects.filter(pk=subscription.pk).update(
            summaries_used=F("summaries_used") + 1
        )
        subscription.refresh_from_db(fields=["summaries_used"]) # type: ignore
